...
public class EndpointServiceImp implements EndpointService {
	...
	public void delete(int id) {
		Optional<Endpoint> entityExists = repository.findById(id);
		if (entityExists.isPresent()) {
			entityExists.get().getAplicacion().getEndpoints().remove(entityExists.get());
			for (Tag tag : entityExists.get().getTags()) {
				tag.getEndpoints().remove(entityExists.get());
			}
			repository.delete(entityExists.get());
		} else {
			throw new IllegalArgumentException(message);
		}
	}
